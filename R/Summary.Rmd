...
```{r address, results='markup', echo=FALSE}

# gets close but adds comment symbols which would need to be removed later
address <- paste("$A",
                 "Food Statistics team",
                 "Department for Environment, Food and Rural Affairs",
...
file.copy(from = here("img", "agrifoodchain-infographic.png"),
          to = here("output", "figures", "agrifoodchain-infographic.png"),
          overwrite = TRUE)
...
```{r !!1-tfp, echo=FALSE, fig.path="../output/figures/", include=FALSE}
...
cols <- c(gov_cols[1], "grey", "grey", "grey", "grey")
names(cols) <- levels(tfp$sector)
colscale <- scale_colour_manual(name = "sector", values = cols)

p1 <- ggplot(data=tfp, aes(x=year, y=productivity, colour = sector)) +
  geom_line(aes(group = sector)) +
  geom_point(aes(group = sector)) +
  theme_gov(axes = "xy", base_size = 10) +
  colscale +
...
cols <- c("grey", gov_cols[1], "grey", "grey", "grey")
names(cols) <- levels(tfp$sector)
colscale <- scale_colour_manual(name = "sector", values = cols)

p2 <- ggplot(data=tfp, aes(x=year, y=productivity, colour = sector)) +
  geom_line(aes(group = sector)) +
  geom_point(aes(group = sector)) +
  theme_gov(axes = "xy", base_size = 10) +
  colscale +
...
cols <- c("grey", "grey", gov_cols[1], "grey", "grey")
names(cols) <- levels(tfp$sector)
colscale <- scale_colour_manual(name = "sector", values = cols)

p3 <- ggplot(data=tfp, aes(x=year, y=productivity, colour = sector)) +
  geom_line(aes(group = sector)) +
  geom_point(aes(group = sector)) +
  theme_gov(axes = "xy", base_size = 10) +
  colscale +
...
cols <- c("grey", "grey", "grey", gov_cols[1], "grey")
names(cols) <- levels(tfp$sector)
colscale <- scale_colour_manual(name = "sector", values = cols)

p4 <- ggplot(data=tfp, aes(x=year, y=productivity, colour = sector)) +
  geom_line(aes(group = sector)) +
  geom_point(aes(group = sector)) +
  theme_gov(axes = "xy", base_size = 10) +
  colscale +
...
cols <- c("grey", "grey", "grey", "grey", gov_cols[1])
names(cols) <- levels(tfp$sector)
colscale <- scale_colour_manual(name = "sector", values = cols)

p5 <- ggplot(data=tfp, aes(x=year, y=productivity, colour = sector)) +
  geom_line(aes(group = sector)) +
  geom_point(aes(group = sector)) +
  theme_gov(axes = "xy", base_size = 10) +
  colscale +
...
```{r chart, echo=FALSE, fig.path="../output/figures/"}
